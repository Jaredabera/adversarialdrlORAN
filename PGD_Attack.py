...
def apply_pgd_attack(data, epsilon=0.01, num_iterations=1000, alpha=0.01):
    perturbed_data = np.copy(data)

    for _ in range(num_iterations):
        perturbation = np.random.uniform(-epsilon, epsilon, size=data.shape)
        perturbed_data = perturbed_data + alpha * np.sign(perturbation)

        # Clip perturbed data to ensure it stays within epsilon bounds
        perturbed_data = np.clip(perturbed_data, data - epsilon, data + epsilon)
...
if __name__ == '__main__':
    ...
    previous_metrics = ''
    # Initialize rewards_dict outside the loop
    rewards_dict = {profile: [] for profile in slice_profiles.keys()}  
    ...
    while True:
        ...
        max_iterations = 1000  # Maximum number of attack iterations
        alpha = 0.01  # Step size for PGD

        for i, profile in enumerate(slice_profiles.keys()):  # Fix: Added enumeration
            if len(data_tmp[i]) > 0:
                for row in data_tmp[i]:
                 row[0] /= 100000

        # Apply the PGD attack to the observations
            perturbed_data = apply_pgd_attack(data_tmp[i], epsilon=epsilon, num_iterations=max_iterations, alpha=alpha)
